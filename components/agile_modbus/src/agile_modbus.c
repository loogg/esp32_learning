 ...
 * @return  数据元长度
 */
int agile_modbus_compute_meta_length_after_function(agile_modbus_t *ctx, int function, agile_modbus_msg_type_t msg_type)
{
    int length = -1;
 ...
 * @return  数据长度
 */
int agile_modbus_compute_data_length_after_meta(agile_modbus_t *ctx, uint8_t *msg, int msg_length, agile_modbus_msg_type_t msg_type)
{
...
static int agile_modbus_receive_msg_judge(agile_modbus_t *ctx, uint8_t *msg, int msg_length, agile_modbus_msg_type_t msg_type)
{
    ...
    rc = agile_modbus_compute_meta_length_after_function(ctx, msg[ctx->backend->header_length], msg_type);
    if (rc < 0)
        return -1;
    remain_len -= rc;
    if (remain_len < 0)
    ...
    rc = agile_modbus_compute_data_length_after_meta(ctx, msg, msg_length, msg_type);
    if (rc < 0)
        return -1;
    remain_len -= rc;
    if (remain_len < 0)
...
void agile_modbus_set_compute_meta_length_after_function_cb(agile_modbus_t *ctx,
                                                            int (*cb)(agile_modbus_t *ctx, int function,
